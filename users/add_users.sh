...
read -p "Создание пользователя завершено. Хотите продолжить добавление пользователя в группы? (да/нет): " continue_choice

if [[ "$continue_choice" =~ ^[дД][аА]|[yY][eE][sS]$ ]]; then
